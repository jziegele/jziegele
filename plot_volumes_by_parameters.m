...
function plot_volumes_by_parameters(filter_params, show_constants_in_title)
% plot_volumes_by_parameters - Filters simulations and plots volume waveforms.
%
% Syntax: plot_volumes_by_parameters(filter_params, show_constants_in_title)
%
% Inputs:
...
%% 3. Plot the Volume Waveforms
figure('Position', [100, 100, 1200, 800]);
volume_names = {'V11 (Epi Art.)', 'V21 (Epi Ven.)', 'V12 (Mid Art.)', 'V22 (Mid Ven.)', 'V13 (Endo Art.)', 'V23 (Endo Ven.)'};
volume_indices = [10, 13, 11, 14, 12, 15]; % Note the order for plotting

for v_idx = 1:6
    subplot(3, 2, v_idx);
    hold on;
    grid on;
    title(volume_names{v_idx});
    xlabel('Time (s)');
    ylabel('Volume (mL)');
    legend_entries = {};
    ...
    for i = 1:length(subset_indices)
        ...
        % --- Extract Variables ---
        t = data.results.t;
        Volume = data.results.o(:,volume_indices(v_idx));
        ...
        % --- Plot ---
        plot(t(t_idx) - t(find(t_idx,1)), Volume(t_idx), 'LineWidth', 1.5);
    ...
    if v_idx == 1 % Only add legend to the first subplot
...
title_str = 'Volume Waveform Comparison';
